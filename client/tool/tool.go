...
func Screen() {
	...
	fmt.Println("3、输入：[private]用户名:内容 私聊...")
...
func RegisterOrLogin(n string, conn net.Conn) *msg.Message {
	...
	for {
		...
		if n == "1" {
			loginMes = &msg.Message{
				Type:    msg.MessageRegister,
				Sender:  username,
				Content: password,
			}
		} else {
			loginMes = &msg.Message{
				Type:    msg.MessageJoin,
				Sender:  username,
				Content: password,
			}
		}
...
func SendServer(content string, conn net.Conn, userMsg *msg.Message, clientQuitFlag chan struct{}) {
	if content == "quit" {
		quitErr := msg.SendJsonMessage(conn, &msg.Message{
	...
	if strings.HasPrefix(content, "[private]") {
		...
		if len(parts) != 2 { // 分成两部分
			fmt.Println("格式错误，应为 [private]用户名:内容")
			return
		}
		// [private]name:context
		target := strings.TrimPrefix(parts[0], "[private]")
		privateErr := msg.SendJsonMessage(conn, &msg.Message{
			Type:     msg.MessagePrivate,
			Sender:   userMsg.Sender,
			Receiver: target,
			Content:  parts[1],
		})
	...
	if content == "rank" {
		rankErr := msg.SendJsonMessage(conn, &msg.Message{
			Type:   msg.MessageRank,
			Sender: userMsg.Sender},
		)
		if rankErr != nil {
	...
	r := msg.SendJsonMessage(conn, &msg.Message{
